...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchScrums = async () => {
      try {
        ...
        setError('Error fetching scrums: ' + error.message);
  ...
  const handleGetDetails = async (scrumId) => {
    try {
      ...
      setError('Error fetching scrum details: ' + error.message);
  ...
  const handleAddScrum = async (event) => {
    ...
    // Basic validation
    if (!newScrumName || !newTaskTitle || !newTaskDescription || !newTaskAssignedTo) {
    ...
    try {
      ...
      // Add new Task and use the response
      ...
      setError('Error adding scrum or task: ' + error.message);
